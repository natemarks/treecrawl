...
class TestData(object):
    ...
    This object is instantiated once per test. It copies the project test data
    (PTD) into temp directories created for each test by pytest.tmp_path. the
    copied data, temporary test data, (TTD) are used by the tests as input and
    expected content.

    Each pytest contains multiple cases. Each case has it's own data set with
    predictably located artifacts. The relative paths to these artifacts are
    stored i in TestData class attributes (DNA_DIR, etc)

    test_manifest.TestData.copy_test_data_to_temp  copies the case directories
    from the project to the temporary dirs for testing
    ...
    def __init__(self, test_name):
        ...
        # temp_test_data is the directory in tmp_path that contains all the
        # cases for this test
        self.temp_test_data = None

    def artifact_path_for_case(self, case, artifact_path):
        """ Absolute path to temporary test data for the current test and case.

        join the tmp directory with the test case name and the constant
        relative path to the artifact

        :param str case: the current test case :param str artifact_path: the
        path to the artifact relative to the temp + case directory

        NOTE: TestData users should just use the class attribute they want as
        the artifact_path parameter example: expected_manifest_markdown_file =
        test_data.artifact_path_for_case(test_case,
        TestData.EXPECTED_MANIFEST_MD)
    ...
    def files_to_compare(self, test_case) -> List[Tuple[str, str]]:
        """Return list of pairs of objects that assert can compare

        Args:
            test_case (str): The test case used to find the initial/ expected
            dirs to look in for files to compare
        ...
        initial_dir = os.path.join(
            self.temp_test_data, *[test_case, "initial"]
        )
