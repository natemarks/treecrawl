...
"""
import os
from .utility import file_to_string, find_path_to_subdirectory, mkdir_p
...
class CaseHelper(object):
    ...
    def __init__(
        ...
        self.golden = os.path.join(
            test_data, test_name, test_case, "golden")
        self.expected = os.path.join(
            temp_dir, test_name, test_case, "expected")
        self.input = os.path.join(
            test_data, test_name, test_case, "input")
        self.actual = os.path.join(
            temp_dir, test_name, test_case, "actual")
    ...
    def _populate_temp(self):
        ...
        if not os.path.isdir(self.golden):
            msg = "Golden is missing: {}. Are you running with "\
    ...
    def compare(self):
        ...
        If self.update_golden is True, golden needs to be updated externally by running
        the function under test and pointing its output to the golden path.  compare deals with this by conditionally run
